package com.project.container.containersapp.frame.presenter.boxing;
...
public class BoxingListPresenter extends Presenter<IBaseListView<BoxingListBean>> {
    ...
    public BoxingListPresenter(Context context, IBaseListView<BoxingListBean> mvpView) {
    ...
    private void requestData() {
        ...
        mRequest.url(HttpContstants.ZX_LIST)
                .params(mParams)
                .clazz(BoxingListBean.class)
                .get()
                .callback(new UserCallback() {
                    @Override
                    public void onSuccess(ResponseData result) {
                        if ("200".equals(result.code)) {
                            BoxingListBean boxingListBean = (BoxingListBean) result.data;
                            ...
                            List<BoxingListBean> list = boxingListBean.list;
                            if (list != null && list.size() > 0) {
                                mvpView.onHasNext(boxingListBean.isHasNext());
                                if (isLoadMore) {

 ...
 */
package com.project.container.containersapp.frame.utils.easypermissions;
...
public class EasyPermissions {
    ...
    public static boolean hasPermissions(Context context, String... perms) {
        ...
        for (String perm : perms) {
            boolean hasPerm = isPermissionGranted(context, perm);
            if (!hasPerm) {
                return false;
            }
        }
    ...
    private static boolean shouldShowRequestPermissionRationale(Object object, String perm) {
        if (object instanceof Activity) {
            ...
            //fragmet的 shouldShowRequestPermissionRationale 方法总是返回false,so 加个判断
    ...
    private static void executePermissionsRequest(Object object, String[] perms, int requestCode) {
        checkCallingObjectSuitability(object);
        if (object instanceof Activity) {
            String[] deniedPrems = checkDeniedPerm((Activity) object, perms);
            if (deniedPrems == null || deniedPrems.length == 0){
                return;
            }
            ActivityCompat.requestPermissions((Activity) object, deniedPrems, requestCode);
        } else if (object instanceof Fragment) {
            String[] deniedPrems = checkDeniedPerm(((Fragment) object).getActivity(), perms);
            if (deniedPrems == null || deniedPrems.length == 0){
                return;
            }
            ((Fragment) object).requestPermissions(deniedPrems, requestCode);
        } else if (object instanceof android.app.Fragment) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                String[] deniedPrems = checkDeniedPerm(((android.app.Fragment) object).getActivity(), perms);
                if (deniedPrems == null || deniedPrems.length == 0){
                    return;
                }
                ((android.app.Fragment) object).requestPermissions(deniedPrems, requestCode);
            }
        }
    }
     ...
     * @return
     */
    private static String[] checkDeniedPerm(Context contexts, String[] perms) {
        if (contexts == null){
            return perms;
        }
        ArrayList<String> deniedPerm = new ArrayList<>();
        for (String perm:perms){
            boolean hasPerm = isPermissionGranted(contexts, perm);
            if (!hasPerm){
                deniedPerm.add(perm);
            }
        }
        return deniedPerm.toArray(new String[deniedPerm.size()]);
    }
